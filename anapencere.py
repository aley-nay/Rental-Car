...
class anapencere(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.sqlBaglan()#sql bağlantısı
        self.dbCheck()#veritabanı kontrolü
        self.tableCheck()#tablo var mı

###############################     anapenceredeki butonlar başlar      ####################################################


        #anapencere sayfasındaki butonları fonksiyonlara bağladım
...
###############################     anapenceredeki butonlar başlar      ####################################################
    ...
    def tableCheck(self):
        ...
        dbTable = [dbTbl[0] for dbTbl in self.mycursor.fetchall()]#table var mı yok mu
        if dbTable == []:
            ...
            if 'bilgi' not in dbTable:
                self.mycursor.execute("""
                    CREATE TABLE bilgi(
                        ...
                        ad varchar(50) NOT NULL,
                        soyad varchar(50) NOT NULL,
                        arac_id int(10) NOT NULL,
                        model varchar(50) NOT NULL,
                        marka varchar(50) NOT NULL,
                        yolculuk varchar(50) NOT NULL,
                        kira varchar(50) NOT NULL
                    );
                """)
    def closeEvent(self, event): #sayfayı kapatınca db de kapansın. arkada boş boş çalışmasın
        self.mycursor.close()
        self.dbConn.close()

###############################     müşteri sayfası başlar      ####################################################
class addPerson(QMainWindow):
    def __init__(self, mainWindow, mycursor, parent=None):
        ...
        self.ui.tableWidget.setHorizontalHeaderLabels( #table widgettaki sütun adlarını değiştirmek için
            ["userID", "Ad", "Soyad", "telno", "dTarihi", "adres", "meslek", "ehliyetSınıfı", "egitimDurumu", "tcno",
             "medeniDurumu"])
        #görünüm için width ver
        ...
        self.ui.tableWidget.setColumnWidth(11, 100)
          ...
          #musteri bilgieri sayfasındaki butonları fonksiyonlara bağladım
...
class Bul(QMainWindow):
    def __init__(self, mainWindow, mycursor, parent=None):
        ...
        self.ui.tableWidget.setColumnWidth(10, 100)
        self.ui.tableWidget.setColumnWidth(11, 100)
        self.ui.pushButton_yeni.clicked.connect(self.yeniFonk1)
        self.ui.pushButton_sil.clicked.connect(self.silFonk1)
        self.ui.pushButton_update.clicked.connect(self.updateFonk1)
        self.ui.pushButton_kaydet.clicked.connect(self.kaydetFonk1)
